  ...
  switch (currentGameState) {
    ...
    case gameState.ENDING:
      ...
      if (flashPlane.visible) {
        flashPlane.material.opacity = Math.min(
          flashPlane.material.opacity + 0.05,
          1,
        );
      }
      break;
  ...
  if (character) {
    ...
    const levitationAnimation = () => {
      ...
      character.position.y =
        startY + (targetY - startY) * easeInOut(verticalProgress);
      character.position.z =
        startZ + (targetZ - startZ) * easeInOut(forwardProgress);
...
function updateWall() {
  ...
  switch (currentGameState) {
    ...
    case gameState.SLOWING:
      ...
      currentSpeed = THREE.MathUtils.lerp(currentSpeed, MIN_SPEED, 0.05);
...
function checkCollision(car, character) {
  ...
  const xCollision =
    Math.abs(carPos.x - characterPos.x) <
    (carBox.width + characterBox.width) / 2;
  const zCollision =
    Math.abs(carPos.z - characterPos.z) <
    (carBox.depth + characterBox.depth) / 2;
...
function handleKeyDown(e) {
  if (e.key === "t") {
    if (tposeAllowed) {
      ...
      actions.tpose.play();
      actions.tpose.setEffectiveTimeScale(0.8).setEffectiveWeight(1.0);
  ...
  if (e.key === "ArrowUp") {
    if (actions && actions.jump) {
      if (activeAction !== actions.jump) {
        ...
        actions.jump.play();
        actions.jump.setEffectiveTimeScale(0.8).setEffectiveWeight(1.0);
